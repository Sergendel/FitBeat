...
from backend.core.user_prompt_utils import prompt_to_audio_params
...
def test_prompt_to_audio_params():
    params, _ = prompt_to_audio_params("relaxing music")
...
def test_filter_tracks_by_audio_params(sample_dataset):
    ...
    assert (
        ...
        >= 1
    ), "At least one track should exactly match original criteria"
