...
def prompt_to_audio_params(user_prompt):
    """
    Converts a user's emotional/situational prompt explicitly into numeric audio
    parameters using an LLM.
    ...
    if not params or not folder_name:
        ...
        print(
            f"\n****   The tracks will be stored in the folder '"
            f"{folder_name}'   *****.\n"
        )
...
def parse_user_prompt_to_dataframe(user_prompt):
    """
    Parses user-provided tracks explicitly from a textual prompt into a
    structured DataFrame.

    Parameters:
        user_prompt (str):
                     Text input explicitly provided by the user containing tracks.

    Returns:
        pd.DataFrame: DataFrame explicitly structured with columns like:
            - artists
            - track_name
            - popularity, tempo, explicit, danceability, etc.
              (set explicitly to None placeholders)
    """
