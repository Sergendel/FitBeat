...
def find_semantically_similar_songs(query, collection, top_k=5):
    """
    Find and return the top-k semantically similar songs from the ChromaDB
    embedding collection, based on the provided free-text query.


    Parameters:
        query (str):
            A textual query describing desired char-stics or examples of the tracks.
    ...
    Returns:
        tuple:
            ...
            - metadatas (list[dict]):
                      Associated metadata (artist, track_name, genre) for each song.

    Example:
         documents, metadata =
                      find_semantically_similar_songs("calm piano music", top_k=3)
    """
...
def get_or_create_song_embedding(artist: str, track_name: str, collection) -> object:
    ...
    If the song isn't available locally, dynamically fetch context from Genius API,
    generate the embedding, and add it to the corpus.
...
if __name__ == "__main__":
    ...
    documents, metadata = find_semantically_similar_songs(
        query, collection=collection_for_test, top_k=5
    )
